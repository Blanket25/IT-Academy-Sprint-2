...
ORDER BY
  ...
  p.apellido2,
  p.nombre;
-- Esbrina el nom i els dos cognoms dels alumnes que no han donat d' alta el seu número
  -- de telèfon en la base de dades.
SELECT
  p.nombre,
  p.apellido1,
  apellido2
...
FROM
  ...
  INNER JOIN alumno_se_matricula_asignatura a ON p.id = a.id_alumno
WHERE
  p.fecha_nacimiento > '1999/01/01'
  AND p.fecha_nacimiento < '1999/12/31';
--!no funciona
  -- Retorna el llistat de professors que no han donat d 'alta el seu número de telèfon en
...
-- Retorna el llistat de les assignatures que s' imparteixen en el primer quadrimestre,
  -- en el tercer curs del grau que t é l 'identificador 7.
SELECT
  *
FROM
  asignatura
WHERE
  ...
  AND curso = 3
  AND id_grado = 7;
-- Retorna un llistat dels professors juntament amb el nom del departament al qual estan
  -- vinculats. El llistat ha de retornar quatre columnes, primer cognom, segon cognom, nom
  -- i nom del departament. El resultat estarà ordenat alfabèticament de menor a major pels
  -- cognoms i el nom.
SELECT
  pe.apellido1,
  pe.apellido2,
...
FROM
  profesor pr
  INNER JOIN persona pe ON pe.id = id_profesor
  INNER JOIN departamento d ON d.id = pr.id_departamento
ORDER BY
  pe.apellido1,
  pe.apellido2,
  pe.nombre;
-- Retorna un llistat amb el nom de les assignatures, any d' inici i any de fi del curs
...
FROM
  ...
  INNER JOIN curso_escolar ce ON ce.id = al.id_curso_escolar
WHERE
  p.nif = '26902806M';
-- Retorna un llistat amb el nom de tots els departaments que tenen professors que
...
FROM
  persona p
  INNER JOIN alumno_se_matricula_asignatura al ON p.id = al.id_alumno
  INNER JOIN curso_escolar ce ON ce.id = al.id_curso_escolar
  INNER JOIN asignatura a ON a.id = al.id_asignatura
WHERE
  ce.anyo_inicio >= 2018
  AND ce.anyo_fin <= 2019;
-- Retorna un llistat amb els noms de tots els professors i els departaments que tenen
  -- vinculats.El llistat tamb é ha de mostrar aquells professors que no tenen cap departament
  -- associat.El llistat ha de retornar quatre columnes,nom del departament, primer cognom,
  -- segon cognom i nom del professor.El resultat estar à ordenat alfab è ticament de menor
  -- a major pel nom del departament, cognoms i el nom.
SELECT
  d.nombre AS nombre_departamento,
  pe.apellido1,
  pe.apellido2,
...
FROM
  profesor pr
  INNER JOIN persona pe ON pe.id = pr.id_profesor
  LEFT JOIN departamento d ON d.id = pr.id_departamento -- Retorna un llistat amb els departaments que no tenen professors associats.
